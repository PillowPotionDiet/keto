...
function hide(arr) {
    ...
    for (let i = 0; i < element.length; i++) {
        // Skip CTA and ATC sections so they remain visible on load
        if (element[i].closest('.atc-button') ||
            element[i].closest('.fade-container') ||
            element[i].closest('.fe-atc-zindex') ||
            element[i].closest('.atc')) {
            continue;
        }
...
function animationSimple(dom, animation) {/*arr = DOM элемент*/
    ...
    window.addEventListener('scroll', function () {
        for (let item of arr) {
            // Do not animate CTA or ATC sections
            if (item.closest('.atc-button') ||
                item.closest('.fade-container') ||
                item.closest('.fe-atc-zindex') ||
                item.closest('.atc')) {
                continue;
            }
